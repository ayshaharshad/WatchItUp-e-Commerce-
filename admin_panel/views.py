...
# @superuser_required
# def order_detail(request, order_id):  # ← Make sure function name is order_detail
#     """Enhanced order detail with item-level management"""
#     order = get_object_or_404(
#         Order.objects.select_related('user').prefetch_related(
#             'items__product',
#             'items__variant',
...
# def cancel_order_items_admin(request, order_id):
#     """
#     ✅ NEW: Admin can cancel specific items from an order
#     """
#     order = get_object_or_404(Order, order_id=order_id)
...
#                     # Calculate refund (only if payment was completed)
#                     refund_amount = Decimal('0')
#                     if order.payment_status == 'completed':
#                         refund_amount = item.item_total
#                         total_refund += refund_amount
...
#                 # Process refund to wallet if needed
#                 if total_refund > 0:
#                     wallet = Wallet.objects.get_or_create(user=order.user)[0]
#                     wallet.add_money(
#                         amount=total_refund,
...
# @superuser_required
# def update_order_status(request, order_id):
#     """Update order status - ALWAYS stays in admin panel"""
#     order = get_object_or_404(Order, order_id=order_id)
...
#             # ✅ FIXED: Handle different status transitions
            
#             # 1. DELIVERED STATUS - Set delivered timestamp
#             if new_status == 'delivered' and not order.delivered_at:
#                 order.delivered_at = timezone.now()
            
#             # 2. CANCELLED STATUS - Restore stock
...
#             # 3. RETURN_APPROVED STATUS - Prepare for return
#             elif new_status == 'return_approved' and old_status == 'return_requested':
...
#             # 4. ✅ NEW: RETURNED STATUS - Complete the return process
#             elif new_status == 'returned' and old_status in ['return_approved', 'return_requested', 'delivered']:
#                 # Restore stock
#                 for item in order.items.filter(status='active'):
#                     if item.variant:
...
#             # 5. REFUNDED STATUS - Just mark as refunded
#             elif new_status == 'refunded':
#                 order.payment_status = 'refunded'
            
#             # 6. If order was cancelled and now being reactivated
#             if old_status == 'cancelled' and new_status in ['confirmed', 'pending', 'processing']:
#                 # Deduct stock again
#                 for item in order.items.filter(status='cancelled'):
#                     if item.variant:
...
#             return redirect(reverse('admin_panel:order_detail', kwargs={'order_id': order.order_id}))
#         else:
#             messages.error(request, "Please correct the errors below.")
#     else:
#         form = OrderStatusForm(current_status=order.status)
...
#     try:
#         with transaction.atomic():
#             # Update return request status to approved (not completed yet)
#             return_request.status = 'approved'
#             return_request.reviewed_by = request.user
#             return_request.reviewed_at = timezone.now()
#             return_request.approved_at = timezone.now()
#             return_request.admin_notes = admin_notes
#             # ✅ DON'T set refund_status to 'processed' yet - wait for 'returned' status
#             return_request.save()
            
#             # Update order status to return_approved (not returned yet)
#             order = return_request.order
#             order.status = 'return_approved'
#             order.save()
