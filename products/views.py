...
# def product_detail(request, uuid):
#     """COMPLETE FIX: Product detail with proper general/variant data separation"""
#     try:
#         product = get_object_or_404(Product, uuid=uuid)
...
#     variants = product.variants.filter(is_active=True).prefetch_related('images').order_by('color')
#     has_variants = variants.exists()
#     default_variant = variants.first() if has_variants else None
    
#     # Get best offer
#     best_offer = product.get_best_offer()
    
#     # ====================================
#     # ✅ GENERAL PRODUCT DATA (Overview)
#     # ====================================
#     if has_variants:
#         general_price_display = product.price_range  # Uses the property from your model
#         general_stock = product.total_stock
#         general_sku = f"WU-{product.id:04d}"
#         general_is_in_stock = product.is_in_stock
#     else:
#         general_price_display = f"₹{product.base_price}"
#         general_stock = product.stock
#         general_sku = f"WU-{product.id:04d}"
#         general_is_in_stock = product.stock > 0
    
#     # ====================================
#     # ✅ DEFAULT VARIANT DATA (Initial display)
#     # ====================================
#     if default_variant:
#         variant_price = default_variant.price
#         variant_stock = default_variant.stock_quantity
#         variant_sku = default_variant.sku
#         variant_is_in_stock = default_variant.is_in_stock
        
#         # Apply offers
#         if best_offer:
#             variant_offer_price = best_offer.get_discounted_price(variant_price)
#             variant_discount_percent = best_offer.discount_percentage
#         else:
#             variant_offer_price = None
#             variant_discount_percent = None
#     else:
#         variant_price = product.base_price
#         variant_stock = product.stock
#         variant_sku = general_sku
#         variant_is_in_stock = product.stock > 0
...
#         # General data
#         'general_price_display': general_price_display,
#         'general_stock': general_stock,
#         'general_sku': general_sku,
#         'general_is_in_stock': general_is_in_stock,
        
#         # Variant data
#         'variant_price': variant_price,
#         'variant_stock': variant_stock,
#         'variant_sku': variant_sku,
#         'variant_is_in_stock': variant_is_in_stock,
#         'variant_offer_price': variant_offer_price,
#         'variant_discount_percent': variant_discount_percent,
...
#         * Empty variant_color = Add base product (uses total variant stock)
#         * Specified variant_color = Add specific variant
#     """
#     try:
#         product = get_object_or_404(Product, uuid=uuid)
...
#                 })
#         else:
#             # CASE 3: Product has variants BUT user didn't select specific variant
#             # Add base product (no specific variant)
#             variant_to_add = None
#             # Use total stock from all variants
#             from django.db.models import Sum
#             total_variant_stock = active_variants.aggregate(
...
#                 cart_item.quantity = product.max_quantity_per_order
#                 cart_item.save()
#                 messages.warning(request, f"Quantity adjusted to maximum allowed: {product.max_quantity_per_order}")
            
#             if quantity > available_stock:
#                 cart_item.quantity = available_stock
#                 cart_item.save()
#                 messages.warning(request, f"Quantity adjusted to available stock: {available_stock}")
...
#             'wallet_balance': wallet_balance,
#             'can_use_wallet': can_use_wallet,
#             'is_partial_checkout': bool(selected_item_ids),  # ✅ Flag for template
#             'selected_items_count': len(selected_item_ids) if selected_item_ids else cart.total_items,
#         }
...
# # ✅ UPDATED: Clear selected items after order placement
# @login_required
# @require_POST
# def place_order_cod(request):
#     """Place order with Cash on Delivery - WITH OFFER SUPPORT AND SELECTED ITEMS"""
...
# @login_required(login_url='users:login')  
# def order_detail(request, order_id):
#     """Order detail page - USER FACING (products app)"""
#     # Ensure user can only see their own orders
#     order = get_object_or_404(
#         Order.objects.prefetch_related('items__product', 'items__variant'),
#         order_id=order_id,
